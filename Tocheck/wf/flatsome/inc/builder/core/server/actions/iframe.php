...
add_filter( 'post_content', function ( $content, $post_id, $context ) {
  if ( current_user_can( 'edit_post', $post_id ) && array_key_exists( 'edit_post_id', $_GET ) && intval( $_GET['edit_post_id'] ) == $post_id ) {
...
add_filter( 'get_post_metadata', function ( $value, $object_id, $meta_key, $single ) {
  ...
  if ( $post_id && $post_id == $object_id && current_user_can( 'edit_post', $post_id ) ) {
...
add_action( 'the_posts', function ( $posts, $query ) {
  ...
  if ( empty ( $posts ) && ( array_key_exists( 'page_id', $_GET ) || array_key_exists( 'p', $_GET ) ) ) {
    $fetched_post = get_post( isset( $_GET['page_id'] ) ? $_GET['page_id'] : $_GET['p'] );
    if ( $fetched_post && current_user_can( 'read_post', $fetched_post->ID ) ) {
      $posts[] = $fetched_post;
    }
  }
    // Check if this post is beeing edited in builder.
  if ( count( $posts ) == 1 && isset( $_GET['post_id'] ) && $posts[0]->ID == intval( $_GET['post_id'] ) ) {
