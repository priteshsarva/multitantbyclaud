...
<?php
// Example: get external image URL from custom field 'featuredimg'
$external_img = get_post_meta( $product_id, 'featuredimg', true );

// Fallback to product thumbnail if external image is empty
if ( ! empty( $external_img ) ) {
    $img_html = '<img src="' . esc_url( $external_img ) . '" alt="' . esc_attr( $_product->get_name() ) . '" class="attachment-woocommerce_thumbnail size-woocommerce_thumbnail" />';
} else {
    $img_html = $_product->get_image(); // default thumbnail
}

if ( ! $product_permalink ) {
    echo $img_html;
} else {
    printf( '<a href="%s">%s</a>', esc_url( $product_permalink ), $img_html );
}
?>
</td>
...
$clean_product_name = ucwords(str_replace('_', ' ', ltrim($product_name, '_')));

// Output safely
echo wp_kses_post( $clean_product_name . '&nbsp;' );
...
// 							echo wp_kses_post( apply_filters( 'woocommerce_cart_item_name', sprintf( '<a href="%s">%s</a>', esc_url( $product_permalink ), $_product->get_name() ), $cart_item, $cart_item_key ) );
						$raw_title = $_product->get_name();

// Remove leading underscores, replace with spaces, and capitalize
$clean_title = ucwords(str_replace('_', ' ', ltrim($raw_title, '_')));

echo wp_kses_post(
    apply_filters(
        'woocommerce_cart_item_name',
        sprintf('<a href="%s">%s</a>', esc_url($product_permalink), esc_html($clean_title)),
        $cart_item,
        $cart_item_key
    )
);
