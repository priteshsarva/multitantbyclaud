...
// Flatsome Products
function ux_products_child($atts, $content = null, $tag = '' ) {
	$sliderrandomid = rand();
...
// 											echo woocommerce_get_product_thumbnail($image_size);
										?>
										<?php
				// ✅ 1. Custom featured image from meta (fallback to Woo default)
				$featured_img_url = get_post_meta($product->get_id(), 'featuredimg', true);
				if (!empty($featured_img_url)) {
					echo '<img loading="lazy" decoding="async" src="' . esc_url($featured_img_url) . '" data-src="' . esc_url($featured_img_url) . '" alt="' . esc_attr($product->get_title()) . '"  />';
				} else {
					// fallback to WooCommerce product image
					echo woocommerce_get_product_thumbnail($image_size);
				}

				// ✅ 2. Hover image (from second image in "imageUrl" JSON)
				...
				if (!empty($gallery_images)) {
					...
					if (is_array($gallery_images) && count($gallery_images) >= 2) {
						...
						echo '<img loading="lazy" decoding="async" src="' . $hover_img . '" data-src="' . $hover_img . '" alt="Alternative view of ' . esc_attr($product->get_title()) . '" class="show-on-hover absolute fill hide-for-small back-image lazy-load-active" aria-hidden="true" style="width:100%;height:auto;object-fit:cover;" />';
					}
				}
				?>
									</a>
...
add_shortcode('ux_products', 'ux_products_child');
